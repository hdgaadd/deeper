...
- **Java的引用**
  ...
  - 强引用
    ...
    若内存不足，JVM宁愿抛出异常，也不会回收该对象；把该对象赋值为**null**则会回收该对象
...
- **左连接与子查询的效率比较**
    ...
    - 数据量大情况下，左连接效率更高
        ...
        子查询需要遍历多次，子查询遍历与连接遍历，而连接查询只需遍历一次
    
- **虚拟列**

    - 当约束条件是对某个字段的**特殊处理**时，直接全表扫描需要对每个字段进行特殊处理，查询效率低下

      可以使用虚拟列，在表中创建该字段的特殊处理后的索引，则查询时可直接使用该索引，而不用对字段特殊处理

    - 字段内容为**JSON**时，难以加索引，可对该JSON的某位置字符串创建虚拟列索引
...
- **LRU**
  ...
  - 当Redis的**缓存空间不足**时，若有新数据添加，需要将**最不可能访问**的数据删除掉
  
- **Redis集合有没有大小限制**

  - 有
  - 参考String类型，**一个字符串**最多可存储512MB**容量**
  - 参考list类型，该集合里最多可包含2^32-1个**元素**
